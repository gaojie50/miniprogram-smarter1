...
const calcValue = (paramValue, digits) => Math.floor(paramValue * Math.pow(10, digits)) / Math.pow(10, digits);
...
const assignDeep = (...objects) => objects.reduce((accumulator, currentValue) => {
...
function formatEndTime(endTime) {
  let year = endTime ? dayjs(endTime).format('YYYY年') : '';
  const hour = endTime ? dayjs(endTime).format('HH:ss') : '';
  const date = endTime ? dayjs(endTime).format('MM月DD日') : '';
  const weekDay = endTime ? calcWeek(endTime) : '';
  ...
  if (dayjs(endTime).isToday()) {
    return {
      year: '',
      date: '今天',
      hour,
      text: `今天/${weekDay} ${hour}`,
    };
  }
