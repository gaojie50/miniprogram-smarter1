...
function getMaoyanSignLabel(arr = []) {
  ...
  let produce = arr.includes(1) ? 1 :
    arr.includes(5) ? 5 :
    arr.includes(3) ? 3 : undefined;
  let release = arr.includes(2) ? 2 :
    arr.includes(6) ? 6 :
    arr.includes(4) ? 4 : undefined;

  return [produce, release].filter(item => !!item).map(item => MaoyanSignSet.filter(inner => inner.value == item)[0].label);
};
